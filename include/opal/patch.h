...
class OpalMediaPatch : public PObject
{
  ...
  //@{
    ...
    /**Propagate the command specified further in the media command chain.
       The commands are highly context sensitive, for example 
       VideoFastUpdate would only apply to a video data stream.
       Usually, media commands are executed by calling the similar
       function on the media stream.

       The default behaviour passes the command along the media command
       chain until the command is consumed
      */
  ...
  protected:
    ...
    class Sink : public PObject {
      ...
      public:
        ...
        void SetCommandNotifiers();
        PDECLARE_NOTIFIER(OpalMediaCommand, OpalMediaPatch::Sink, ExecuteCommandFromPrimaryCodec);
        PDECLARE_NOTIFIER(OpalMediaCommand, OpalMediaPatch::Sink, ExecuteCommandFromSecondaryCodec);
