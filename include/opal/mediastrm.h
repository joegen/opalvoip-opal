 ...
 *   outgoing codecs
 *
 * Revision 2.40.2.3  2007/02/13 11:30:59  hfriederich
 ...
 * consumed.
 *
 * Revision 2.40.2.1  2007/02/07 08:51:01  hfriederich
...
class OpalMediaStream : public PObject
{
  ...
  //@{
    ...
    /**Execute the command specified. The commands are highly context 
       sensitive, for example VideoFastUpdate would only apply
       to a video transcoder.
        
       The default behaviour passes the command on the media patch if
       isEndOfChain is false. If isEndOfChain is true, this stream's
       command notifier is invoked if present.
      */
    virtual BOOL ExecuteCommand(
      const OpalMediaCommand & command,    ///< Command to execute.
      BOOL isEndOfChain = FALSE            ///< Indicates which direction to take
    );

    /**Set a notifier to receive media commands arriving through the 
       media command chain.

       The default behaviour sets the member variable commandNotifier.
      */
    virtual void SetCommandNotifier(
      const PNotifier & notifier    ///<  notifier to call.
    );
