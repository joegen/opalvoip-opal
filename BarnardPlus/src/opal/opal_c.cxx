...
void OpalManager_C::HandleMediaStream(const OpalMessage & command, OpalMessageBuffer & response)
{
  ...
  if (!IsNullString(command.m_param.m_mediaStream.m_type)) {
    if (strstr(command.m_param.m_mediaStream.m_type, "audio") != NULL)
      sessionID = OpalMediaFormat::DefaultAudioSessionID;
    else if (strstr(command.m_param.m_mediaStream.m_type, "video") != NULL)
      sessionID = OpalMediaFormat::DefaultVideoSessionID;
    else if (strstr(command.m_param.m_mediaStream.m_type, "fax") != NULL)
      sessionID = OpalMediaFormat::DefaultDataSessionID;
    source = strstr(command.m_param.m_mediaStream.m_type, "out") != NULL;
  }
  ...
  if (!IsNullString(command.m_param.m_mediaStream.m_identifier))
    ...
    stream = connection->GetMediaStream(sessionID, source);
  else {
  ...
  switch (command.m_param.m_mediaStream.m_state) {
    case OpalMediaStateOpen :
      if (sessionID == 0)
        ...
        if (source)
          call->OpenSourceMediaStreams(*connection, sessionID, mediaFormat);
        else
          call->OpenSourceMediaStreams(*call->GetOtherPartyConnection(*connection), sessionID, mediaFormat);
      }
      break;
