...
bool H263_Base_EncoderContext::Open(CodecID codecId)
{
  ...
  _inputFrameBuffer = (unsigned char *)FFMPEGLibraryInstance.AvMalloc(MAX_YUV420P_FRAME_SIZE);
  if (_inputFrameBuffer == NULL) {
    TRACE_AND_LOG(tracer, 1, "Failed to allocate aligned input frame");
    return false;
  }
...
{
  FFMPEGLibraryInstance.AvFree(_inputFrame);
  FFMPEGLibraryInstance.AvFree(_inputFrameBuffer);
}
...
void H263_Base_EncoderContext::SetTSTO (unsigned tsto)
{
  ...
  _context->qmax = (int)ceil ( (31.0 - H263P_MIN_QUANT) / 31.0 * tsto + H263P_MIN_QUANT);
...
H263_RFC2190_EncoderContext::~H263_RFC2190_EncoderContext()
{
  ...
  if (FFMPEGLibraryInstance.IsLoaded()) {
    ...
    FFMPEGLibraryInstance.AvFree(_context);
  }  
...
int H263_RFC2190_EncoderContext::EncodeFrames(const BYTE * src, unsigned & srcLen, BYTE * dst, unsigned & dstLen, unsigned int & flags)
{
  ...
  _inputFrame->data[0] = OPAL_VIDEO_FRAME_DATA_PTR(header);
  _inputFrame->data[1] = _inputFrame->data[0] + size;
  _inputFrame->data[2] = _inputFrame->data[1] + (size / 4);
...
H263_Base_DecoderContext::~H263_Base_DecoderContext()
{
  if (FFMPEGLibraryInstance.IsLoaded()) {
    ...
    FFMPEGLibraryInstance.AvFree(_context);
    FFMPEGLibraryInstance.AvFree(_outputFrame);
  }
}
...
bool H263_RFC2190_DecoderContext::DecodeFrames(const BYTE * src, unsigned & srcLen, BYTE * dst, unsigned & dstLen, unsigned int & flags)
{
  ...
  int gotPicture = 0;
  int bytesDecoded = FFMPEGLibraryInstance.AvcodecDecodeVideo(_context, _outputFrame, &gotPicture, &depacketizer.frame[0], (int)depacketizer.frame.size());
...
static int to_normalised_options(const struct PluginCodec_Definition *, void * _context, const char *, void * parm, unsigned * parmLen)
{
  ...
  int mpi[5];
  int minWidth, minHeight, maxHeight, maxWidth, frameTime, maxBitRate, targetBitRate;
...
static int to_customised_options(const struct PluginCodec_Definition *, void * _context, const char *, void * parm, unsigned * parmLen)
{
  if (parmLen == NULL || parm == NULL || *parmLen != sizeof(char ***))
    return 0;
