...
PBoolean SIP_PDU::Read(OpalTransport & transport)
{
  ...
  PStringStream transportString; transportString << transport;
  ParseCodes parseCode = Read(*stream, 
                              transport.IsReliable() ? 1000000 : datagram.GetLength(),
                              (const char *)transportString);
  switch (parseCode) {
    case Parse_OK:
      break;

    case Parse_InvalidDatagram:
      if (stream == &datagram) {
  ...
  return parseCode == Parse_OK;
}

SIP_PDU::ParseCodes SIP_PDU::Read(istream & stream, PINDEX maxReadLength, const char * transportString)
{
  if (transportString == NULL)
    transportString = "(no transport)";
  ...
  if (cmd.Left(4) *= "SIP/") {
    ...
    if (cmds.GetSize() < 3) 
      return Parse_BadRequestLine;

    int i = 0;
    while (!(cmds[0] *= MethodNames[i])) {
      i++;
      if (i >= NumMethods) 
        return Parse_UnknownMethodName;
    }
    method = (Methods)i;
  ...
  if (versionMajor < 2) 
    return Parse_InvalidVersion;

  // Getthe MIME fields
  stream >> mime;
  if (!stream.good() || mime.IsEmpty()) 
    return Parse_InvalidMIME;
  ...
  if (!contentLengthPresent) {
    PTRACE(2, "SIP\tNo Content-Length present from " << transportString << ", reading till end of datagram/stream.");
  }
  else if (contentLength < 0) {
    PTRACE(2, "SIP\tImpossible negative Content-Length from " << transportString << ", reading till end of datagram/stream.");
    contentLengthPresent = false;
  }
  else if (contentLength > maxReadLength) {
    PTRACE(2, "SIP\tImplausibly long Content-Length " << contentLength << " received from " << transportString << ", reading to end of datagram/stream.");
    contentLengthPresent = false;
  }
