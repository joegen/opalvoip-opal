...
PBoolean MyManager::Initialise(PArgList & args)
{
  ...
  ///////////////////////////////////////
  // Create RTSP protocol handler
  if (!args.HasOption("no-rtsp")) {
    rtspEP = new OpalRTSPEndPoint(*this);
    if (!InitialiseRTSPEP(args, rtspEP))
      return PFalse;
  }
...
#if OPAL_HAS_RTSP
    if (rtspEP != NULL) {
      AddRouteEntry("pots:.*\\*.*\\*.* = rtsp:<dn2ip>");
      AddRouteEntry("pots:.*           = rtsp:<da>");
      AddRouteEntry("pc:.*             = rtsp:<da>");
    }
#endif
...
PBoolean MyManager::InitialiseRTSPEP(PArgList & args, OpalRTSPEndPoint * rtspEP)
{
  PString prefix = rtspEP->GetPrefixName();

  // Start the listener thread for incoming calls.
  PStringArray listeners = args.GetOptionString("rtsp-listen").Lines();
  if (!rtspEP->StartListeners(listeners)) {
    cerr <<  "Could not open any " << prefix << " listener from "
         << setfill(',') << listeners << endl;
    return false;
  }
  cout << prefix << " listeners: " << setfill(',') << rtspEP->GetListeners() << setfill(' ') << endl;
