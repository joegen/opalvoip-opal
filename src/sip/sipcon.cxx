 ...
 *
 * Revision 2.197.2.6  2007/03/30 06:45:21  hfriederich
 * Fixed GCC warning
 *
...
RTP_UDP *SIPConnection::OnUseRTPSession(const OpalMediaType & mediaType, const OpalTransportAddress & mediaAddress, OpalTransportAddress & localAddress)
{
...

  {
    ...
    if (ownerCall.IsMediaBypassPossible(*this, mediaType)) {
      ...
      rtpSession = (RTP_UDP *)UseSession(GetTransport(), mediaType, NULL);
      if (rtpSession == NULL) {
        return NULL;
      }
                                              ...
                                              SDPSessionDescription & sdpOut)
{
  ...
  SDPMediaDescription * incomingMedia = sdpIn.GetMediaDescription(mediaType);
  if (incomingMedia == NULL) {
    PTRACE(2, "SIP\tCould not find matching media type for session " << mediaType);
    return FALSE;
  }
  
  if (incomingMedia->GetMediaFormats(mediaType).GetSize() == 0) {
    PTRACE(1, "SIP\tCould not find media formats in SDP media description for session " << mediaType);
    return FALSE;
  }
  ...
  remoteFormatList += incomingMedia->GetMediaFormats(mediaType);
...

  {
    ...
    rtpSession = OnUseRTPSession(mediaType, mediaAddress, localAddress);
    if (rtpSession == NULL && !ownerCall.IsMediaBypassPossible(*this, mediaType)) {
    ...
    localMedia = new SDPMediaDescription(localAddress, mediaType.GetMIMEMediaType());
  
    // create map for RTP payloads
    incomingMedia->CreateRTPMap(mediaType, rtpPayloadMap);

    // Open the streams and the reverse media streams
    BOOL reverseStreamsFailed = TRUE;
    reverseStreamsFailed = OnOpenSourceMediaStreams(remoteFormatList, mediaType, localMedia);
  
    // Add in the RFC2833 handler, if used
    if (hasTelephoneEvent) {
      SDPCapability * capability = capabilities.FindCapability(OpalRFC2833);
      if (capability != NULL) {
        localMedia->AddMediaFormat(*capability, rtpPayloadMap);
      }
    }
#if OPAL_T38FAX
    if (hasNSE) {
    ...
    if (reverseStreamsFailed) {
      ...
      if (!BuildSDP(sdp, rtpSessions, mediaType)) {
        delete localMedia;
        return FALSE;
      }
      return TRUE;
...
BOOL SIPConnection::OnOpenSourceMediaStreams(const OpalMediaFormatList & remoteFormatList, const OpalMediaType & mediaType, SDPMediaDescription *localMedia)
{
  BOOL reverseStreamsFailed = TRUE;

  ownerCall.OpenSourceMediaStreams(*this, remoteFormatList, mediaType);
                                          ...
                                          const OpalMediaType & mediaType)
{
  ...
  switch (remoteSDP.GetDirection(mediaType)) {
    case SDPMediaDescription::Inactive :
    case SDPMediaDescription::RecvOnly :
      return FALSE;

    default :
      return OpalConnection::OpenSourceMediaStream(mediaFormats, mediaType);
  }
}
...
OpalMediaStream * SIPConnection::OpenSinkMediaStream(OpalMediaStream & source)
{
  ...
  switch (remoteSDP.GetDirection(source.GetMediaType())) {
    case SDPMediaDescription::Inactive :
                             ...
                             RTP_SessionManager & rtpSessions)
{
  ...
  for(PINDEX i = 0; i < mediaTypes.GetSize(); i++) {
    sdpSuccess = BuildSDP(sdp, rtpSessions, mediaTypes[i]) || sdpSuccess;
  }
                             ...
                             const OpalMediaType & mediaType)
{
  ...
  for (i = 0; i < formats.GetSize(); i++) {
    ...
    if (fmt.GetMediaType() == mediaType)
      break;
  }
  if (i >= formats.GetSize()) {
    PTRACE(3, "SIP\tNo media formats for media type " << mediaType << ", not adding SDP");
    return FALSE;
  }
  ...
  if (localAddress.IsEmpty()) {
    ...
    RTP_Session * rtpSession = rtpSessions.UseSession(mediaType);
    if (rtpSession == NULL) {

      // Not already there, so create one
      rtpSession = CreateSession(GetTransport(), mediaType, NULL);
      if (rtpSession == NULL) {
...
BOOL SIPConnection::ConstructSDP(SDPSessionDescription & sdpOut, BOOL releaseOnFailure)
{
  BOOL sdpFailure = TRUE;

  // get the remote media formats, if any
  if (originalInvite->HasSDP()) {
    // try to build an answer to the offer
    SDPSessionDescription & sdpIn = originalInvite->GetSDP();
    sdpFailure = !OnSendSDPMediaDescription(sdpIn, sdpOut);
  }
	
  if (sdpFailure) {
    // build a new offer
    SDPSessionDescription *sdp = (SDPSessionDescription *) &sdpOut;
    sdpFailure = !BuildSDP(sdp, rtpSessions);
...

  {
    ...
    for (PINDEX i = 0; i < forkedInvitations.GetSize(); i++) {
      if (&forkedInvitations[i] == &transaction) {
        forkedInvitations.RemoveAt(i); 
        if (forkedInvitations.GetSize() == 0) {
          release = TRUE;
        }
        break;
      }
  ...
  if (transaction.GetMethod() == SIP_PDU::Method_INVITE) {
    ...
    if (!reInvite && response.GetStatusCode()/100 <= 2) {
      if (response.GetStatusCode()/100 == 2) {
        ...
        PWaitAndSignal m(invitationsMutex);
        for (i = forkedInvitations.GetSize(); i > 0; i--) {
          if (&forkedInvitations[i-1] != &transaction) {
            forkedInvitations[i-1].Cancel();
            forkedInvitations.RemoveAt(i-1);
          }
        }
...
void SIPConnection::OnReceivedReINVITE(SIP_PDU & request)
{
  ...
  if (originalInvite->HasSDP()) {
    ...
    // FIXME: Make work for all media types
    if (sdpIn.GetDirection(OpalDefaultAudioMediaType) == SDPMediaDescription::SendOnly &&
        sdpIn.GetDirection(OpalDefaultVideoMediaType) == SDPMediaDescription::SendOnly) {
  ...
  BOOL success = ConstructSDP(sdpOut, FALSE);
...
BOOL SIPConnection::OnOpenIncomingMediaChannels()
{
  ...
  if (originalInvite != NULL) {
    ...
    OpalMediaTypeList mediaTypes = OpalMediaType::GetAllRegisteredMediaTypes();
    PINDEX i;
    for (i = 0; i < mediaTypes.GetSize(); ++i) {
      SDPMediaDescription * mediaDescription = sdp.GetMediaDescription(mediaTypes[i]);
      if (mediaDescription != NULL) {
        previewFormats += mediaDescription->GetMediaFormats(mediaTypes[i]);
        OpalTransportAddress mediaAddress = mediaDescription->GetTransportAddress();
        mediaTransportAddresses.SetAt(mediaTypes[i].GetUniqueID(), new OpalTransportAddress(mediaAddress));
      }
    }
...
void SIPConnection::OnReceivedSDP(SIP_PDU & pdu)
{
  ...
  for (PINDEX i  = 0; i < mediaTypes.GetSize(); i++) {
    ...
    if (mediaTypes[i] == OpalDefaultAudioMediaType) {
      remoteFormatList += OpalRFC2833;
    }
  }
}
                                                  ...
                                                  const OpalMediaType & mediaType)
{
  ...
  if (mediaDescription == NULL) {
    PTRACE(mediaType.GetMIMEMediaType() <= OpalMediaType::Video ? 2 : 3, "SIP\tCould not find SDP media description for " << mediaType);
    return FALSE;
  }
...

  {
    ...
    if (!OnOpenSourceMediaStreams(remoteFormatList, mediaType, NULL))
      return FALSE;
  }
...
{
  PTRACE(4, "SIP\tJob handler thread started.");
  ...
  PTRACE(4, "SIP\tJob handler thread finished.");
}
...
BOOL SIPConnection::SendUserInputTone(char tone, unsigned duration)
{
  ...
  PTRACE(3, "SIP\tSendUserInputTime('" << tone << "', " << duration << "), using mode " << mode);
