 ...
 *
 * $Log: t38proto.cxx,v $
 * Revision 1.2010.2.4  2007/05/28 16:41:46  hfriederich
 * Backport from HEAD, changes since May 3, 2007
 *
 * Revision 2.9.2.3  2007/05/04 08:17:54  hfriederich
 * Fix compilation
 *
...
const OpalMediaType FaxAudioMediaType("FaxAudio", OpalMediaType::Audio);
...
{
  if (shutdownWrite) {
    PTRACE(3, "RTP_T38\tSession " << mediaType << ", Write shutdown.");
    shutdownWrite = FALSE;
    return FALSE;
  }
  ...
  while (!dataSocket->WriteTo(frame.GetPointer(),
    ...
    switch (dataSocket->GetErrorNumber()) {
      case ECONNRESET :
      case ECONNREFUSED :
        PTRACE(2, "RTP_T38\tSession " << mediaType << ", data port on remote not ready.");
        break;

      default:
        PTRACE(1, "RTP_T38\tSession " << mediaType
...
BOOL T38PseudoRTP::ReadData(RTP_DataFrame & frame, BOOL loop)
{
  do {
    ...
    if (shutdownRead) {
      PTRACE(3, "T38_RTP\tSession " << mediaType << ", Read shutdown.");
      shutdownRead = FALSE;
      return FALSE;
    }
    ...
    switch (selectStatus) {
      ...
      case PSocket::Interrupted:
        PTRACE(3, "T38_RTP\tSession " << mediaType << ", Interrupted.");
        return FALSE;

      default :
        PTRACE(1, "T38_RTP\tSession " << mediaType << ", Select error: "
...
OpalFaxMediaStream::OpalFaxMediaStream(OpalConnection & conn, const OpalMediaFormat & mediaFormat, BOOL isSource, const PString & _token, const PString & _filename)
  : OpalMediaStream(conn, mediaFormat, isSource), sessionToken(_token), filename(_filename)
{
  faxCallInfo = NULL;
  SetDataSize(300);
}
...
OpalT38MediaStream::OpalT38MediaStream(
  ...
  : OpalFaxMediaStream(conn, mediaFormat, isSource, token, _filename)
{
}
...
OpalMediaStream * OpalFaxConnection::CreateMediaStream(const OpalMediaFormat & mediaFormat, BOOL isSource)
{
  ...
  if (mediaFormat.GetMediaType() == OpalDefaultAudioMediaType) {
    if (forceFaxAudio && (mediaFormat == OpalPCM16))
      return new OpalFaxMediaStream(*this, mediaFormat, isSource, GetToken(), filename);
    else
      return new OpalNullMediaStream(*this, mediaFormat, isSource);
  }

  // if creating a data stream, see what type it is
  else if (!forceFaxAudio && (mediaFormat.GetMediaType() == FaxAudioMediaType)) {
...
BOOL OpalFaxConnection::OpenSourceMediaStream(const OpalMediaFormatList & mediaFormats, const OpalMediaType & mediaType)
{
#if OPAL_VIDEO
...
{
#if OPAL_VIDEO
  if (source.GetMediaFormat().GetMediaType() == OpalDefaultVideoMediaType)
    return NULL;
#endif
...
OpalMediaStream * OpalT38Connection::CreateMediaStream(const OpalMediaFormat & mediaFormat, BOOL isSource)
{
  // if creating an audio session, use a NULL stream
  if (mediaFormat.GetMediaType() == OpalDefaultAudioMediaType) 
    return new OpalNullMediaStream(*this, mediaFormat, isSource);
  ...
  // if creating a data stream, see what type it is
  else if (mediaFormat == OpalT38)
    return new OpalT38MediaStream(*this, mediaFormat, isSource, GetToken(), filename);
