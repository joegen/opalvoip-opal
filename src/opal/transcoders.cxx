...
void OpalTranscoder::NotifyCommand(const OpalMediaCommand & command) const
{
  if (commandNotifier != PNotifier())
    ...
    PTRACE(4, "Transcoder\tNo command notifier available for transcoder " << this);
}
...
PBoolean OpalTranscoder::ConvertFrames(const RTP_DataFrame & input, RTP_DataFrameList & output)
{
  ...
  if (!AcceptComfortNoise() && (pt == RTP_DataFrame::CN || pt == RTP_DataFrame::Cisco_CN)) {
    PTRACE(5, "Transcoder\tRemoving comfort noise frame with payload type " << pt);
    return true;
  }
  ...
  if (pt != m_lastPayloadType) {
    ...
    PTRACE(2, "Transcoder\tConsecutive mismatched payload type, expected "  << GetPayloadType(true) << ", now using " << pt);
    inputMediaFormat.SetPayloadType(pt);
    return Convert(input, outframe);
  }

  PTRACE(4, "Transcoder\tRemoving frame with mismatched payload type " << pt << " - should be " << GetPayloadType(true));
  return true;
}
                                                       ...
                                                       unsigned instanceLen)
{
  ...
  if (transcoder == NULL) {
    PTRACE2(2, NULL, "Transcoder\tCould not create transcoder instance from " << srcFormat << " to " << destFormat);
    return NULL;
  }
  ...
  PTRACE2(2, transcoder, "Transcoder\tError creating transcoder instance from " << srcFormat << " to " << destFormat);
  delete transcoder;
  return NULL;
}
                         ...
                         OpalMediaFormat & dstFormat)
{
  ...
  if (masterFormat == masterFormats.end()) {
    ...
    PTRACE(5, "Transcoder\tInitial source format from capabilities:\n" << setw(-1) << srcFormat);
  }
  else {
    srcFormat = *masterFormat;
    PTRACE(5, "Transcoder\tInitial source format from master:\n" << setw(-1) << srcFormat
  ...
  if (masterFormat == masterFormats.end()) {
    ...
    PTRACE(5, "Transcoder\tInitial destination format from capabilities:\n" << setw(-1) << dstFormat);
  }
  else {
    dstFormat = *masterFormat;
    PTRACE(5, "Transcoder\tInitial destination format from master:\n" << setw(-1) << dstFormat
