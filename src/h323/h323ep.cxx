...
template <class PDUClass>
static void CallOnOutgoingClearToken(H323Connection & connection, H323SignalPDU & signalPDU)
{
  PDUClass & pdu = signalPDU.m_h323_uu_pdu.m_h323_message_body;
  H225_ArrayOf_ClearToken & tokens = pdu.m_tokens;
  connection.OnOutgoingClearToken(tokens, signalPDU);
  if (tokens.GetSize() > 0)
    pdu.IncludeOptionalField(PDUClass::e_tokens);
}

BOOL H323EndPoint::OnSendConnect(H323Connection & connection,
                                 H323SignalPDU & connectPDU)
{
  CallOnOutgoingClearToken<H225_Connect_UUIE>(connection, connectPDU);
  return TRUE;
}
...
BOOL H323EndPoint::OnSendSignalSetup(H323Connection & connection,
                                     H323SignalPDU & setupPDU)
{
  CallOnOutgoingClearToken<H225_Setup_UUIE>(connection, setupPDU);
  return TRUE;
}
...
{
  CallOnOutgoingClearToken<H225_CallProceeding_UUIE>(connection, callProceedingPDU);
  return TRUE;
}
...
{
  CallOnOutgoingClearToken<H225_Alerting_UUIE>(connection, alerting);
...
void H323EndPoint::OnOutgoingClearToken(H323Connection & conn, H225_ArrayOf_ClearToken & tokens, H323SignalPDU & pdu)
{
  ...
  tokens[last].m_tokenOID = H235_ClearToken_V3;
#endif
}
...
static BOOL BuildH235SRTPKey(H323Connection & conn, OpalSRTPSecurityMode & srtpMode, PBYTEArray & opaque)
{
  ...
  PTRACE(4, "H225\tBuild SRTP_SrtpKeys:\n  " << setprecision(2) << srtpKeys);

  // encode into PER
  PPER_Stream strm;
  srtpKeys.Encode(strm);
...
BOOL H323EndPoint::OnSendSRTPOffer(H323Connection & conn, const H323_RTPChannel & channel,H245_OpenLogicalChannel & param)
{
  ...
  H235_V3KeySyncMaterial & v3KeySyncMaterial = h235Key;

  PASN_ObjectId & algoId = v3KeySyncMaterial.m_algorithmOID;
  algoId.SetValue(H235_SRTP_ObjectId);

  v3KeySyncMaterial.IncludeOptionalField(H235_V3KeySyncMaterial::e_genericKeyMaterial);
  v3KeySyncMaterial.m_genericKeyMaterial.SetValue(srtpKeys);

  PTRACE(4, "H225\tBuild V3KeySyncMaterial:\n  " << setprecision(2) << v3KeySyncMaterial);
  ...
  // put SRTP information into OLC
  param.IncludeOptionalField(H245_OpenLogicalChannel::e_encryptionSync);
  H245_EncryptionSync & encryptionSync = param.m_encryptionSync;
  encryptionSync.m_h235Key.SetValue(strm);
  encryptionSync.m_synchFlag = channel.GetTransmittedRTPPayloadType();

  return TRUE;
}

#if P_SSL
BOOL H323SecureEndPoint::OnSendSRTPOffer(H323Connection & conn, const H323_RTPChannel & channel,H245_OpenLogicalChannel & param)
{
  return TRUE;
}
#endif  // P_SSL
